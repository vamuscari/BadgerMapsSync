DROP VIEW IF EXISTS AccountsIndexedColumns;
CREATE VIEW AccountsIndexedColumns AS
...
custom_bounds AS (
    SELECT MAX(cid) AS last_custom
    FROM account_columns
    WHERE is_custom = 1
)
SELECT
    CAST(COALESCE(NULLIF(data_set.Label, ''), account_column.name) AS TEXT) AS Name,
    CAST(COALESCE(NULLIF(data_set.Type, ''), account_column.type) AS TEXT) AS Type,
